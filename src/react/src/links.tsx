...
import type { ExplorerName } from "./explorers";
 ...
 */
export const LinkToExplorer: React.FC<
	ExplorerLinkProps & {
		explorer: ExplorerName;
	}
> = ({ explorer, network, kind, addr, html = {}, children = null }) => {
	...
	return <LinkExternal {...html}>{children || shortenAddress(addr)}</LinkExternal>;
};
...
const createExplorerLinkComponent = (
	...
	return ({ network, kind, addr, html = {}, children = null }: ExplorerLinkProps) => {
		html.href = makeUrl(network, kind, addr);
		return <LinkExternal {...html}>{children || shortenAddress(addr)}</LinkExternal>;
	};
};
