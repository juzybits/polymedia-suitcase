...
export class SuiMultiClient {
	 ...
	 */
	constructor(endpointUrls: string[], rateLimitDelay = 300) {
		this.clients = [];
		this.clientIdx = 0;
	...
	public async executeInBatches<InputType, OutputType>(
		...
		const retries: InputType[] = [];
		const batchSize = this.clients.length;
		const totalBatches = Math.ceil(inputs.length / batchSize);
		onUpdate?.(
			`[SuiMultiClient] Executing ${inputs.length} operations in batches of ${batchSize}`,
		);

		for (
			let start = 0, batchNum = 1;
			start < inputs.length;
			start += batchSize, batchNum++
		) {
			...
			batchResults.forEach((result, index) => {
				if (result.status === "fulfilled") {
					...
					onUpdate?.(
						`[SuiMultiClient] ERROR. status: ${result.status}, reason: ${result.reason}`,
					);
