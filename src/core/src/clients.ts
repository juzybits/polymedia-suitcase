...
export async function devInspectAndGetReturnValues<T extends any[]>(
	...
	if (blockParsers.length !== blockResults.length) {
		throw new Error(
			`You provided parsers for ${blockParsers.length} txs but the txblock contains ${blockResults.length} txs`,
		);
	}
	...
	for (const [txIdx, txResult] of blockResults.entries()) {
		if (!txResult.returnValues?.length) {
			throw new Error(
				`Transaction ${txIdx} didn't return any values: ${JSON.stringify(txResult, null, 2)}`,
			);
		}
		...
		if (txParsers.length !== txResult.returnValues.length) {
			throw new Error(
				`You provided parsers for ${txParsers.length} return values but tx ${txIdx} contains ${txResult.returnValues.length} return values`,
			);
		}
...
export async function getSuiObjectRef(
	...
	if (resp.error || !resp.data) {
		throw new Error(
			`[getSuiObjectRef] failed to fetch objectId | error: ${JSON.stringify(resp.error)}`,
		);
	}
	return {
