import type { SuiObjectRef, SuiObjectResponse, SuiParsedData } from "@mysten/sui/client";
 ...
 */
export function objResToContent(resp: SuiObjectResponse): SuiParsedData {
 ...
 */
export function objResToDisplay(resp: SuiObjectResponse): ObjectDisplay {
 ...
 */
export function objResToId(resp: SuiObjectResponse): string {
	if (!resp.data) {
 ...
 */
export function objResToOwner(resp: SuiObjectResponse): string {
	if (!resp.data?.owner) {
 ...
 */
export function objResToRef(resp: SuiObjectResponse): SuiObjectRef {
	if (!resp.data) {
		throw Error(`response has no data: ${JSON.stringify(resp, null, 2)}`);
	}
	return {
